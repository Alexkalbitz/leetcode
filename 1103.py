...
# first person, n + 2 candies to the second person, and so on until we give 2 * n candies to the last person.
#
...
class Solution:
    def distributeCandies(self, candies: int, num_people: int):
        ...
        while candies > 0:
            for person in range(len(candylist)):
                counter += 1
                if candies < 1:
                    return candylist
                if counter > candies:
                    candylist[person] += candies
                    candies -= candies
                else:
                    candylist[person] += counter
                    candies -= counter
        return candylist
